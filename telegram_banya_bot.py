...
async def save_user(message: types.Message, state: FSMContext):
    ...
    if user_name in users:
        ...
        users[user_name] = 0
        expenses[user_name] = {"food": 0, "alcohol": 0}
...
@dp.message(F.text.in_(["🍗 Їжа", "🍾 Алкоголь"]), BathSession.selecting_expense_type)
async def ask_expense_amount(message: types.Message, state: FSMContext):
    category_map = {"🍗 Їжа": "food", "🍾 Алкоголь": "alcohol"}
